...
impl LLVMContext {
	pub fn new(module_name: &str) -> Self {
		unsafe {
			...
			let root_function = LLVMAddFunction(
				module,
				root_function_name.as_ptr(),
	...
	pub fn compile_module_item(
		...
		match module_item {
			...
			ModuleItem::StatementListItem(sli) => match sli {
				...
				boa_ast::StatementListItem::Declaration(_declaration) => {
					todo!()
				}
			},
		}
		...
		match expression {
			...
			Expression::Literal(literal) => match literal {
				boa_ast::expression::literal::Literal::String(string) => {
					let string_value =
						interner.resolve_expect(*string).utf8().unwrap();
				...
				boa_ast::expression::literal::Literal::Bool(bool) => {
					Some(unsafe {
						LLVMConstInt(
